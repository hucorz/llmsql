...
class BackendInterfaceBase:
    ...
    async def inference(
        self, local_messages, request_unique_id: Optional[str]
    ) -> AsyncIterator[str]:
        """
    ...
    def report_last_time_performance(self):
        try:
            ...
            data_query_time = self.profiler.get_timer_sec("data_query")
            prefill_time = self.profiler.get_timer_sec("prefill")
            decode_count = self.profiler.get_counter("decode")
            trie_hit_count = self.profiler.get_counter("trie_hit")
            logger.info(
                f"\n\tCount(T): decode {decode_count}, trie_hit {trie_hit_count}."
...
class ThreadContext:
    ...
    def __init__(
        self, run: RunObject, interface: BackendInterfaceBase, args: ConfigArgs = default_args
    ) -> None:
        self.args = args
        ...
        self.messages = self.message_manager.db_list_messages_of_thread(
            run.thread_id, order=Order.ASC
        )
    ...
    async def work(self) -> AsyncIterator:
        ...
        )  # must get this before we interseted reply_message
