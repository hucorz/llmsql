...
class TransformersInterface(BackendInterfaceBase):
    ...
    def _init(self):
        ...
        self.cache_path = os.path.normpath(os.path.join(script_dir, f"../data/cache.db"))
        conn = sqlite3.connect(self.cache_path)
        cursor = conn.cursor()

        # 查看 db 中有没有表的名字为 model_name, 没有则创建这张表
        cursor.execute(
            f"CREATE TABLE IF NOT EXISTS `{self.model_name}` (hash_key TEXT PRIMARY KEY, cache BLOB)"
        )
        conn.commit()

        # load system cache
        system_hash_key = hashlib.sha256("system_cache".encode("utf-8")).hexdigest()
        cursor.execute(
            f"SELECT cache FROM `{self.model_name}` WHERE hash_key = '{system_hash_key}'"
        )
        result = cursor.fetchone()
        if result:
            self.system_cache = torch.load(io.BytesIO(result[0]), weights_only=True)
        else:
            self.system_cache = _sys_cache_prep(self.model, self.tokenizer)
            cursor.execute(
                f"INSERT INTO `{self.model_name}` (hash_key, cache) VALUES (?, ?)",
                (system_hash_key, self.cache_to_blob(self.system_cache)),
            )
            conn.commit()

    def cache_to_blob(self, cache: torch.Tensor):
        buffer = io.BytesIO()
    ...
    def data_query(
        ...
        if not use_turbo:
            ...
            result = cursor.fetchone()
            if result:
                data_cache = torch.load(io.BytesIO(result[0]), weights_only=True)
            else:
                raise ValueError("Data cache not found")
            # TODO create new data cache
            # else:
            #     data_cache = _cache_prep(
            #         model=self.model,
            #         tokenizer=self.tokenizer,
